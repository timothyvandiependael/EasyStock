...
export class ClientOverview {
  ...
  loadData() {
    ...
    this.getAdvancedSub = this.clientService.getAdvanced(query).subscribe({
      next: (result) => {
  ...
  onRowSelected(row: any) {
    ...
    if (blockBtn) {
      blockBtn.disabled = !this.authService.canDelete("Client");
      if (row.blUserId) {
  ...
  onAddClicked() {
    this.router.navigate(['app/client/edit', 'add']);
  }
  ...
  onEditClicked() {
    ...
    this.router.navigate(['app/client/edit', 'edit', id])
  }
  ...
  executeBlock(id: number) {
    this.blockSub = this.clientService.block(id).subscribe({
      next: () => {
  ...
  executeUnblock(id: number) {
    this.unblockSub = this.clientService.unblock(id).subscribe({
      next: () => {
