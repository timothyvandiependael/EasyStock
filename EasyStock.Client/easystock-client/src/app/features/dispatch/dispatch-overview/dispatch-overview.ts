...
export class DispatchOverview {
  ...
  loadData() {
    ...
    this.getAdvancedSub = this.dispatchService.getAdvanced(query).subscribe({
      next: (result) => {
  ...
  onRowSelected(row: any) {
    ...
    if (editBtn) editBtn.disabled = !this.authService.canEdit("Dispatch");
    const blockBtn = this.buttons.find(b => b.action === 'block' || b.action === 'unblock');
    if (blockBtn) {
      blockBtn.disabled = !this.authService.canDelete("Dispatch");
      if (row.blUserId) {
  ...
  onAddClicked() {
    this.router.navigate(['app/dispatch/edit', 'add']);
  }
  ...
  onEditClicked() {
    ...
    this.router.navigate(['app/dispatch/edit', 'edit', id])
  }
  ...
  executeBlock(id: number) {
    this.blockSub = this.dispatchService.block(id).subscribe({
      next: () => {
      ...
      error: (err) => {
        ...
        this.persistentSnackbar.showError(`Error blocking ${this.selectedRow.dispatchNumber}. If the problem persists, please contact support.`);
      }
    })
  }
  ...
  executeUnblock(id: number) {
    this.unblockSub = this.dispatchService.unblock(id).subscribe({
      next: () => {
      ...
      error: (err) => {
        ...
        this.persistentSnackbar.showError(`Error unblocking ${this.selectedRow.dispatchNumber}. If the problem persists, please contact support.`);
      }
    })
  }
