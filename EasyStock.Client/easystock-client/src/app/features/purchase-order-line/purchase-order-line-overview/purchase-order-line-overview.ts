...
@Component({
  ...
  imports: [DataTable],
  templateUrl: './purchase-order-line-overview.html',
  styleUrl: './purchase-order-line-overview.css'
})
export class PurchaseOrderLineOverview {
  ...
  ngOnInit() {
    ...
    if (addBtn) addBtn.disabled = !this.authService.canAdd("PurchaseOrderLine");
  ...
  loadData() {
    ...
    this.getAdvancedSub = this.purchaseOrderLineService.getAdvanced(query).subscribe({
      next: (result) => {
  ...
  onRowSelected(row: any) {
    ...
    if (blockBtn) {
      blockBtn.disabled = !this.authService.canDelete("PurchaseOrderLine");
      if (row.blUserId) {
  ...
  onEditClicked() {
    ...
    this.router.navigate(['app/purchaseorderline/edit', 'edit', id])
  }
  ...
  executeBlock(id: number) {
    this.blockSub = this.purchaseOrderLineService.block(id).subscribe({
      next: () => {
  ...
  executeUnblock(id: number) {
    this.unblockSub = this.purchaseOrderLineService.unblock(id).subscribe({
      next: () => {
