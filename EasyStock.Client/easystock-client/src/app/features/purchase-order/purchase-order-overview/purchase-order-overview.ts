...
export class PurchaseOrderOverview {
  ...
  onRowSelected(row: any) {
    ...
    if (blockBtn) {
      blockBtn.disabled = !this.authService.canDelete("PurchaseOrder");
      if (row.blUserId) {
  ...
  onAddClicked() {
    this.router.navigate(['app/purchaseorder/edit', 'add']);
  }
  ...
  onEditClicked() {
    ...
    this.router.navigate(['app/purchaseorder/edit', 'edit', id])
  }
  ...
  executeBlock(id: number) {
    this.blockSub = this.purchaseOrderService.block(id).subscribe({
      next: () => {
  ...
  executeUnblock(id: number) {
    this.unblockSub = this.purchaseOrderService.unblock(id).subscribe({
      next: () => {
