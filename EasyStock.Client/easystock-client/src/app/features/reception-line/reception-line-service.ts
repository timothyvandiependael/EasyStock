...
import { ReceptionLineOverviewDto } from './dtos/reception-line-overview.dto';
...
export class ReceptionLineService {
  private readonly apiUrl = environment.receptionLine;
    ...
    getAdvanced(params: AdvancedQueryParametersDto): Observable<PaginationResult<ReceptionLineOverviewDto>> {
      if (params.filters) {
      ...
      return this.http.post<PaginationResult<ReceptionLineOverviewDto>>(this.apiUrl + 'advanced', params);
    }

    private formatBeforePost(receptionLine: CreateReceptionLineDto) {
      if (!receptionLine.quantity) receptionLine.quantity = 0;
    }
  
    add(dto: CreateReceptionLineDto): Observable<ReceptionLineDetailDto> {
      this.formatBeforePost(dto);
      return this.http.post<ReceptionLineDetailDto>(this.apiUrl, dto);
    }
  
    getById(id: number) {
      return this.http.get<ReceptionLineDetailDto>(this.apiUrl + 'id/' + id);
    }
  
    edit(id: number, dto: UpdateReceptionLineDto) {
      this.formatBeforePost(dto);
