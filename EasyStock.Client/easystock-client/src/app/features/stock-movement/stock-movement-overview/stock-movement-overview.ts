...
export class StockMovementOverview {
  ...
  loadData() {
    ...
    this.getAdvancedSub = this.stockMovementService.getAdvanced(query).subscribe({
      next: (result) => {
  ...
  onRowSelected(row: any) {
    ...
    if (editBtn) editBtn.disabled = !this.authService.canEdit("StockMovement");
    const blockBtn = this.buttons.find(b => b.action === 'block' || b.action === 'unblock');
    if (blockBtn) {
      blockBtn.disabled = !this.authService.canDelete("StockMovement");
      if (row.blUserId) {
  ...
  onAddClicked() {
    this.router.navigate(['app/stockmovement/edit', 'add']);
  }
  ...
  onEditClicked() {
    ...
    this.router.navigate(['app/stockmovement/edit', 'edit', id])
  }
  ...
  executeBlock(id: number) {
    this.blockSub = this.stockMovementService.block(id).subscribe({
      next: () => {
      ...
      error: (err) => {
        ...
        this.persistentSnackbar.showError(`Error blocking stock movement with id ${this.selectedRow.id}. If the problem persists, please contact support.`);
      }
    })
  }
  ...
  executeUnblock(id: number) {
    this.unblockSub = this.stockMovementService.unblock(id).subscribe({
      next: () => {
      ...
      error: (err) => {
        ...
        this.persistentSnackbar.showError(`Error unblocking stock movement with id ${this.selectedRow.id}. If the problem persists, please contact support.`);
      }
    })
  }
  ...
  export(format: 'csv' | 'excel') {
    ...
    this.stockMovementService.export(query, format);
  }
